...
export default function MoodScoreChip({
  ...
  return (
    <span className="inline-flex w-fit items-center gap-[7px] rounded-[100px] border border-neutral-95 bg-white px-3 py-1 body-14">
      <div className="body-18 shrink-0">{'☹️'}</div>
      <div className="flex min-w-0 flex-end gap-[3px]">
        {responsiveCompact ? (
          <>
            <span className="whitespace-nowrap text-label-neutral max-[1200px]:hidden">
              {label}
            </span>
            {locale === 'en' ? (
          ...
          <span className="whitespace-nowrap text-label-neutral">{label}</span>
        )}
